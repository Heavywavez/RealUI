...
local skins do
    ...
    skins = {
        ...
        args = {
            ...
            uiScale = {
                name = _G.UI_SCALE,
                desc = _G.OPTION_TOOLTIP_UI_SCALE,
                type = "range",
                min = 0.5, max = 2, step = 0.1,
                validate = function(info, value)
                    value = _G.tonumber(value)
                    debug("Validate uiScale", skins.args[info[#info]].min, value)
                    local option = skins.args[info[#info]]
                    return value >= option.min and value <= option.max
                end,
                get = function() return _G.RealUI_SkinsDB.realUIScale end,
                set = function(info, value) 
                    _G.RealUI_SkinsDB.realUIScale = value
                end,
                order = 30,
            },
            fonts = fonts,
